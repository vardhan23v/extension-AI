  ...
  try {
    ...
    try {
      ...
      if (logoRes.ok) {
        ...
        sanitizedFiles.push({ filename: 'icon128.png', content: logoBase64, encoding: 'base64' });
        sanitizedFiles.push({ filename: 'icon48.png', content: logoBase64, encoding: 'base64' });
        sanitizedFiles.push({ filename: 'icon16.png', content: logoBase64, encoding: 'base64' });
        sanitizedFiles.push({ filename: 'Store_Assets/logo.png', content: logoBase64, encoding: 'base64' });
      }

      // Fetch banner as base64
      const bannerRes = await fetch(bannerUrl);
      if (bannerRes.ok) {
        const bannerBuffer = await bannerRes.arrayBuffer();
        const bannerBase64 = Buffer.from(bannerBuffer).toString('base64');
        sanitizedFiles.push({ filename: 'Store_Assets/banner.png', content: bannerBase64, encoding: 'base64' });
      }

      // Store listing text
      const storeListing = `Title: ${aiResponse.title}\n\nSummary:\n${aiResponse.storeDescription || 'A powerful Chrome Extension.'}\n\nFeatures:\n- Generated by Text-to-Extension AI\n- Manifest V3 Compliant`;
      sanitizedFiles.push({ filename: 'Store_Assets/store_listing.txt', content: storeListing });

    } catch (fetchErr) {
      console.error('Failed to fetch AI generated assets for ZIP:', fetchErr);
    }
